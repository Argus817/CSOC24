...
We are given a python script [`source.py`](source.py) as shown below.
...
The other file [`output.txt`](output.txt) is generated from the above python script and contains Alice's public key. Hence we have the curve, Alice's public key, Bob's public key and the generator point. We can use SageMath's `discrete_log` function to calculate Alice's secret integer and multiply it with Bob's public key to find the shared secret and hence decrypt the flag.
